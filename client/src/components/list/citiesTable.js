...
class Citiestable extends Component {
    ...
    limitIncrease = () => {
        this.props.HousesStore.increatseLimit(this.props.HousesStore.currCity);
    }
            ...
            <React.Fragment>
                <table id="houses-table">
                    ...
                    <tr>
                        ...
                        <th style={stl}><Select name="selectCity" initValue="All Cities" /></th>
    ...
    limitSetting() {
        const {totalNumber, limit, currCity, currentCityCount} = this.props.HousesStore;
        if(currCity === 'default') {
            return limit >= totalNumber ? 'd-none' : 'd-block';
        }else {
            return limit >= currentCityCount ? 'd-none' : 'd-block';
        }
    }
