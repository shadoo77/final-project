...
class HousesStore {
    ...
    @observable averages = {};
    @observable currCity = localStorage.getItem("selectCity") || 'default'
    ...
    @computed get currentCityCount() {
        const count = this.cities.filter(el => {
            return el.location_city === this.currCity && this.currCity !== 'default' ? el.Count : null;
        });
        return count;
    }

    @action changeCurrCity = (city) => {
        this.currCity = city;
    }
